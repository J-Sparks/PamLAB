...
library(readr)
AlphOrderwith9crs_MASTER_CRSCSE_allcourse_tier <- read_csv("AlphOrderwith9crs_MASTER_CRSCSE_allcourse_tier.csv", 
                                                           col_types = cols(X1 = col_skip()))# NAs
glimpse(AlphOrderwith9crs_MASTER_CRSCSE_allcourse_tier)

#NA function
pMISS <- function(x){sum(is.na(x))/length(x)*100}
apply(AlphOrderwith9crs_MASTER_CRSCSE_allcourse_tier, 2, pMISS) #less than 10% CRSNAME1:4


# Create new cols PriorInd and aveGPAInd
AlpOrderTIERS <- AlphOrderwith9crs_MASTER_CRSCSE_allcourse_tier %>% 
  mutate(aveGPAInd=ifelse(FIRST_FALL_GPA >=2.77,"above2.77","below2.77" )) 

aveGPANB<- AlpOrderTIERS %>% 
...
accyAlphbW <- data.frame(models, accuracy_Train, accuracy_Test)
