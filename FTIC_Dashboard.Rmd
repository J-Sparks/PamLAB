...
```{r}
FTIC_ALL_APR_CRD_DEG_DEMO <- read_csv("FTIC_ALL_APR_CRD_DEGV1.csv") %>% select(2,"Cohort","Stu_DEMO_TIME_FRAME.y","Stu_College.x","Stu_Department.x","Stu_College.y","Stu_Department.y","NumFallTerms","GPAAPRind","CRDGPAAPR","Deg_College","Deg_Department","Deg_TermGranted")   %>% mutate(GraduationTime = ifelse((Cohort==2013 & Deg_TermGranted <= 201705)|(Cohort==2014 & Deg_TermGranted <=201805)|(Cohort==2015 & Deg_TermGranted <=201905)|(Cohort==2016 & Deg_TermGranted <=202005)|(Cohort==2017 & Deg_TermGranted <=202105)|(Cohort==2018 & Deg_TermGranted <=202205)|(Cohort==2019 & Deg_TermGranted <=202305), "OnTime","OverTime" )) 
colnames(FTIC_ALL_APR_CRD_DEG_DEMO) <- c("STU_ID","Cohort","STU_Term","Entry_college","Entry_depart","Transitional_college","Transitional_depart","Number_FallTerm","APR","CRDGPAAPR","Deg_college","Deg_depart","Deg_Term","GraduationTime")
 

rpivotTable(FTIC_ALL_APR_CRD_DEG_DEMO[(!is.na(FTIC_ALL_APR_CRD_DEG_DEMO$GraduationTime))&(FTIC_ALL_APR_CRD_DEG_DEMO$Number_FallTerm==1),], aggregatorName = "Count",
            rows = c("Deg_college","Cohort"),
            cols = c("GraduationTime"),
           
            subtotals = T, rendererName = "Table with Subtotal Col Heatmap")
           # vals = "Freq",
          #  rendererName = "Table")
