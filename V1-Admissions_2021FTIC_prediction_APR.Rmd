...
### Cleaning FTIC 2018 & 2019 Banner Application Data
...
imputed_df <- rfImpute(APR~ GPA_HIGHSCHOOL+APR_HSNAME+HS_ENG_YRS+HS_MAT_YRS+HS_SS_YRS+HS_NS_YRS+HS_FL_YRS+CourseWork+APP_NO, data = V3_APR_MASTER, iter=3)
...
V3_APR_MASTER$HS_SS_YRS <- imputed_df$HS_SS_YRS
V3_APR_MASTER$HS_NS_YRS <- imputed_df$HS_SS_YRS
...
V3_APR_MASTER$HS_SS_YRS_index <-  ifelse(V3_APR_MASTER$HS_SS_YRS<=4.0, "SS<=4.0YRS","SS>4.0YRS")
V3_APR_MASTER$HS_NS_YRS_index <-  ifelse(V3_APR_MASTER$HS_NS_YRS<=4.0, "NS<=4.0YRS","NS>4.0YRS")


colSums(is.na(V3_APR_MASTER))

V3_APR_MASTER_vari <- V3_APR_MASTER %>% select(UNIV_ROW_ID,APP_NO,APR_PROGRAM,APR_HSNAME,codeHSGPA,Top5PassRate,
                                               CourseWork,HS_ENG_YRS, HS_MAT_YRS,HS_SS_YRS,HS_NS_YRS,PELL_VERF,HS_MAT_YRS_index,HS_ENG_YRS_index,HS_SS_YRS_index,HS_NS_YRS_index,APP_NO) 
xtabs(~V3_APR_MASTER_vari$APP_NO+ V3_APR_MASTER_vari$HS_MAT_YRS_index)
write.csv(V3_APR_MASTER_vari, "APR_FTIC201819_all.csv")
```
...
set.seed(123)
TrainingIndex <- createDataPartition(V3_APR_MASTER_vari$APR, p=0.7, list = FALSE)
TrainingSet <- V3_APR_MASTER_vari[TrainingIndex,] # Training Set
TestingSet <- V3_APR_MASTER_vari[-TrainingIndex,] # Test Set
...
```{r echo=FALSE}
knitr::kable(tab_1, caption = "Training Set")
```

```{r echo=FALSE}
knitr::kable(tab_2, caption = "Test Set")
```


## Coefficients (reference is HSGPA < 3.10, Gender = Female)
...
results_table <- datatable(FOR_TABLE,extensions = "Buttons",caption = "Probability of APR",
          filter = "top",
